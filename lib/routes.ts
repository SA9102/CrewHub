export const BASE_URL_DEV = "http://localhost:3000"

// URL Routes
export const URL_CREATE_ORGANISATION = "/auth/create-organisation"
export const URL_SIGNIN = "/auth/signin"

// API Routes
export const API_POST_ORGANISATION = "/api/organisation"
export const API_POST_SIGNIN = "/api/auth/signin"
export const API_GET_VALIDATE_INVITE_TOKEN = "/api/auth/validate-invite-token"
export const API_POST_CREATE_USER = "/api/auth/create-user"
export const API_GET_USERS = "/api/users"
export const API_DELETE_USER = "/ap"
